...
export function renderTabelaPontos(pontos) {
	...
	Object.keys(porData)
		...
		.forEach((date) => {
			...
			html += `<div>`
			html += `<div class="text-center text-lg font-bold tracking-wider rounded-t bg-stone-500/15 border-b border-b-black/85 dark:border-b-white/65 dark:text-stone-300 text-black suse ">${date} •${new Date(year, month - 1, day).toLocaleDateString('pt-BR', { weekday: 'short' })} • <span data-less8h=${isLess8h} data-plus8h=${isPlus8h} data-ok=${isOk} class="data-[less8h='true']:dark:text-amber-400 data-[less8h='true']:text-amber-700 data-[plus8h='true']:dark:text-pink-400 data-[plus8h='true']:text-pink-600">${totalHHMM}${isPlus8h ? '!' : ''}</span></div>`
